...
- **The bundled EEZ/gear-type/vessel-type reference data (`src/reference-data/`) is hand-verified against live API calls, not generated from third-party geo datasets.** Don't "fix" this by swapping in Marine Regions/MRGID data wholesale — GFW's internal IDs aren't guaranteed to match. Extend the table the same way it was built: verify each new entry live.
